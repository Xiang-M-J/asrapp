...
void XFFTToVector(XFFT xf, Vector * vp,int power2Flag)
{
	...
	if (power2Flag)N2 = (int)pow(2.0,ceil(log((float)N)/log(2)));
...
//Vector
// number + data  ÿ�������ĳ���Ϊ���ݳ��� + 1
Vector CreateVector(int n)
{
	/*int* i; int j;
	Vector v = (float*)malloc(sizeof(float) * (n + 1));
...
Vector CreateSVector(int n)
{
	...
	Ptr *p;
	int *i;
	p = (Ptr *)malloc(sizeof(float)*(n+1)+sizeof(Ptr)*2);
	v = (SVector)(p + 2);
	i = (int *)v; *i = n;
	SetHook(v, NULL);
	SetUse(v, 0);
...
Matrix CreateMatrix(int nrows, int ncols)
{
	...
	Vector *m;
	char *p;//ʹ��char����Ϊ���ǵ����ֽ�
	p = (char*)malloc(sizeof(float)*((ncols + 1)*nrows) + (nrows + 1) * sizeof(Vector));
	//���е�һ����������,�����Ϊ����,������ĵ�һ�д�������
	i = (int *)p; *i = nrows;
	vsize = (ncols+1)*sizeof(float);
	m = (Vector *)p;
...
SMatrix CreateSMatrix(int nrows, int ncols)
{
	...
	p = (char*)malloc(sizeof(float)*((ncols + 1)*nrows) + (nrows + 3) * sizeof(Vector))+2 * sizeof(Ptr *);
	i = (int *)p; *i = nrows;
	vsize = (ncols + 1) * sizeof(float);
	m = (Vector *)p;
	p += (nrows + 1) * sizeof(Vector);
...
Matrix CreateSTriMat(int size)
{
	...
	Vector *m;
	char *p;
	p = (char *)malloc(size*(sizeof(float) * 2 + (size + 1) * sizeof(float)) / 2+ (size + 1) * sizeof(Vector) + 2 * sizeof(Ptr)) + 2 * sizeof(Ptr *);
	i = (int *)p; *i = size;
	m = (Vector *)p;
	p += (size + 1) * sizeof(Vector);
	for (j = 1; j <= size; j++) {
		i = (int *)p; *i = j;
		m[j] = (Vector)p; p += sizeof(float)*(j+1);
	}
	SetHook(m, NULL);
	SetUse(m, 0);
	return m;
...
{
	int size = STriMatSize(m);int i=0,j=0;float t=0;
	for (i = 1; i <= size; i++) {
