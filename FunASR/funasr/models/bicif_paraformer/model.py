...
class BiCifParaformer(Paraformer):
    ...
    def forward(self, speech, speech_lengths, maxlenratio=0.0, minlenratio=0.0):
        ...
        pre_token_length = pre_token_length.round().long()
        if torch.max(pre_token_length) < 1:
            raise Exception("max value of pre_token_length less than 1")
        decoder_outs = self.cal_decoder_with_predictor(
            encoder_out, encoder_out_lens, pre_acoustic_embeds, pre_token_length
        )
        decoder_out, ys_pad_lens = decoder_outs[0], decoder_outs[1]
    ...
    def export_wav(self, **kwargs):
        from .export_meta import export_rebuild_model_wav

        if "max_seq_len" not in kwargs:
            kwargs["max_seq_len"] = 512
        models = export_rebuild_model_wav(model=self, **kwargs)
        return models
